...
if __name__ == '__main__':
    ...
    for bond_length in np.linspace(0.5,3.0,26):
        ...
        while not better_than_HF:
            if len(results_g)!=0:
                res = minimize(energy, results_g[-1].x, method="SLSQP",tol=1e-8, options={'maxiter':10000}, args=(hamiltonian_dict, size, "0011", k_indices, j_indices, 'paper', True, True))
            else:
                res = minimize(energy, np.random.rand(8), method="SLSQP",tol=1e-8, options={'maxiter':10000}, args=(hamiltonian_dict, size, "0011", k_indices, j_indices, 'paper', True, True))
            print(f'uCJ: {res.fun}')
        ...
        de_g.append(res['fun']-fci_energy)
        ...
        for csf_key in CSFs.keys():
            ...
            overlaps_g[csf_key].append(simulator.overlap(simulator2.state_vector,'IIII'))
        simulators_g.append(simulator) 

    plt.rcParams['font.family'] = 'Serif'
    plt.figure(figsize=(5,5))
    plt.plot(np.linspace(0.5,3.0,26),np.array(de_g),color="royalblue", label="g-uCJ",marker='s',markersize=4)
    ...
    plt.tight_layout()
    plt.savefig("H2_sto3G_g_uCJ_RHF.jpg", dpi=600)       