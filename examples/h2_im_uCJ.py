...
if __name__ == '__main__':
    ...
    for bond_length in np.linspace(0.5,3.0,26):
        ...
        while not better_than_HF:
            if len(results_im)!=0:
                res = minimize(energy, results_im[-1].x, method="SLSQP",tol=1e-8, options={'maxiter':10000}, args=(hamiltonian_dict, size, "0011", k_indices, j_indices, 'paper', False, True))
            else:
                res = minimize(energy, np.random.rand(6), method="SLSQP",tol=1e-8, options={'maxiter':10000}, args=(hamiltonian_dict, size, "0011", k_indices, j_indices, 'paper', False, True))
            print(f'uCJ: {res.fun}')
        ...
        de_im.append(res['fun']-fci_energy)
        
        real = False
        imag = True
        param=results_im[-1]['x']
        initial_state='0011'
        k_indices = [(0,2),(1,3)]
        j_indices = [(0,1),(0,3),(2,1),(2,3)]
        if imag:
        ...
        for csf_key in CSFs.keys():
            ...
            overlaps_im[csf_key].append(simulator.overlap(simulator2.state_vector,'IIII'))
        simulators_im.append(simulator) 

    plt.rcParams['font.family'] = 'Serif'
    plt.figure(figsize=(5,5))
    plt.plot(np.linspace(0.5,3.0,26),np.array(de_im), color="green", alpha=0.8, label="Im-uCJ",marker='^')
    ...
    plt.tight_layout()
    plt.savefig("H2_sto3G_im_uCJ_RHF.jpg", dpi=600)       